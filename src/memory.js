...
function pageLRU() {
  ...
  pages.forEach((page) => {
    ...
    if (foundIndex !== -1) {
      const elementoRemovido = queue.splice(foundIndex, 1)[0];
      queue.push(elementoRemovido);
    }
    if (foundIndex === -1) addPageLRU(page, queue);
  });
  function addPageLRU(valor, tabela) {
    if (tabela.length + 1 == MAX) tabela.shift();
    tabela.push(valor);
  }
  ...
  //   let ram = document.querySelector(`#ram-${page}`);
  ...
  for (let i = 0; i < queue.length; i++) {
    let ram = document.querySelector(`#ram-${i}`);
    ram.innerHTML = `<span class="atual-page">${queue[i]}</span> `;
  }

  for (let i = 0; i < queue.length; i++) {
    let disc = document.querySelector(`#disc-${queue[i]}`);
    disc.innerHTML = `<span class="atual-page">${i}</span> `;
  }
